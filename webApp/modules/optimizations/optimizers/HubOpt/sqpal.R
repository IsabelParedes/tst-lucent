...
# SQPAL : Local Derivative Based optimizer with constraints 

sqpal.run <- function(PbDefinition, ParametersValues, AdvancedParametersValues) {
  library(HubOpt4R) #IFPEN library
  source("optimizers/HubOpt/CreateDefaultXML.R")
  
  ParametersValues$solver = "SQPAL"
...
sqpal.description <- function(){

	DisplayName="HubOpt SQPAL"

	Description="SQPAL performs local derivative-based constrained optimization using a SQP method (with numerical derivatives if not provided)."

	OptimTags=list(constraints=TRUE, categorical=FALSE, monoobj=TRUE, multiobj=FALSE, derivative=TRUE)

	Warnings=list()

	Parameters=list(
	dx_init=list(default=1,lower=0,upper=Inf,type="float",description="Initial perturbation of x."),
	dx_min=list(default=1.e-6,lower=0,upper=Inf,type="float",description="Minimal perturbation of x."),
