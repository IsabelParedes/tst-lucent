...
get.listDOE <- function(Xopt, lb, ub, ntestMC, callback) {
  ...
  Xhalton <- runif.halton(nb.samples,nb.inputs)
  ...
  callback(1)
  Xsobol <- runif.sobol(nb.samples,nb.inputs)
...
evaluateDOE.ui <- function(id) {
  ns <- NS(id)
  tagList(
    fluidRow(actionButton(ns("check"), "Launch Check",icon = icon("chart-bar"), class = "btn-info",width='100%')),
    br(),
    fluidRow(
...
evaluateDOE.server <- function(input, output, session, DOE, settings) {
  ...
  output$plot <- renderPlotly({
    req(DOE$Xopt, DOE$Xinfos, input$check, checkDOE$listDOE, cancelOutput = TRUE)
